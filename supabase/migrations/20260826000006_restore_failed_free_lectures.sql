-- A failed job must not consume a user's one free lecture.
update public.billing_accounts accounts
set free_used = false, updated_at = now()
where free_used
  and not exists (
    select 1 from public.lectures
    where owner_id = accounts.owner_id
      and billing_kind = 'free'
      and status <> 'error'
  );
...
begin
  ...
  if not account.free_used or not exists (select 1 from public.lectures where owner_id = auth.uid() and billing_kind = 'free' and status <> 'error') then
