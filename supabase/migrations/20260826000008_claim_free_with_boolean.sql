...
declare account public.billing_accounts%rowtype; can_use_free boolean;
begin
  ...
  if exists (select 1 from public.lectures where id = p_lecture_id and billed_at is not null) then
    return query select lectures.billing_kind, accounts.stripe_customer_id from public.lectures lectures left join public.billing_accounts accounts on accounts.owner_id = lectures.owner_id where lectures.id = p_lecture_id; return;
  end if;
  ...
  can_use_free := coalesce(account.free_used, false) = false;
  if not can_use_free then select not exists (select 1 from public.lectures where owner_id = auth.uid() and billing_kind = 'free' and status <> 'error') into can_use_free; end if;
  if can_use_free then
    ...
    if account.included_used < 24 then update public.billing_accounts set included_used = included_used + 1, updated_at = now() where owner_id = auth.uid(); update public.lectures set billing_kind = 'included', billed_at = now() where id = p_lecture_id; return query select 'included'::text, null::text; end if;
