create function public.claim_lecture_for_owner_v2(p_lecture_id uuid, p_owner_id uuid)
returns table (kind text, stripe_customer_id text)
language plpgsql security definer set search_path = public as $$
declare subscription text; used integer; customer text; free_lectures integer;
begin
  ...
  select accounts.subscription_status, accounts.included_used, accounts.stripe_customer_id into subscription, used, customer from public.billing_accounts accounts where accounts.owner_id = p_owner_id for update;
  ...
  if subscription in ('active', 'trialing') then
    if used < 24 then update public.billing_accounts set included_used = included_used + 1, updated_at = now() where owner_id = p_owner_id; update public.lectures set billing_kind = 'included', billed_at = now() where id = p_lecture_id; return query select 'included'::text, null::text; end if;
    update public.lectures set billing_kind = 'overage', billed_at = now() where id = p_lecture_id; return query select 'overage'::text, customer;
  end if;
  raise exception 'Your free lecture is used. Subscribe to continue.';
end $$;

alter function public.claim_lecture_for_owner_v2(uuid, uuid) set row_security = off;
