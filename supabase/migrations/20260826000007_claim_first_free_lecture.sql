-- `SELECT ... INTO` can leave the account row null on a first claim.
...
begin
  ...
  if coalesce(account.free_used, false) = false or not exists (select 1 from public.lectures where owner_id = auth.uid() and billing_kind = 'free' and status <> 'error') then
