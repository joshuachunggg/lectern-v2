...
Deno.serve(async request => {
  ...
  try {
    ...
    const transcripts: string[] = [], materials: string[] = [];
    ...
    const response = await openai('/responses', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'gpt-4.1-mini', input: `Create accurate study notes from this lecture transcript and course material. Include an outline, definitions, examples, instructor emphasis, recap, and five active-recall questions. Do not invent facts.\n\nTRANSCRIPT\n${transcript || '[No audio supplied.]'}\n\nMATERIALS\n${context}` }) });
