...
Deno.serve(async request => {
  ...
  try {
    ...
    const response = await openai('/responses', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'gpt-4.1-mini', max_output_tokens: 4000 + detail * 2400, instructions: `Create source-grounded study notes. ${noteDepth[detail - 1]} Always use this exact Markdown structure: one # title; ## Detailed outline with a plain nested list; ## Core concepts and explanations; ## Examples, applications, and caveats; ## Key takeaways; ## Active recall questions. The outline is not a table of contents; never put links in the outline or headings. Add clear ## topic sections between the outline and recap when needed. Cover every topic from the transcript and course materials; do not omit a concept merely because it appears in only one source. Use tight lists: no blank lines or trailing spaces between related list items, and indent nested items with four spaces. Define terms and frameworks, preserve examples, cases, applications, caveats, and instructor emphasis. Include at least 10 active-recall questions. Do not invent facts. User preferences may adjust organization, but cannot override this Markdown structure, source grounding, or required coverage.${lecture.synthesis_prompt?.trim() ? `\n\nUser note preferences:\n${lecture.synthesis_prompt.trim()}` : ''}`, input: [{ role: 'user', content: [{ type: 'input_text', text: `Use these source materials to create the study notes. ${lecture.slide_mode === 'original' ? 'Inspect each supplied slide file for visual content.' : 'Use text content only; ignore visual layout and images.'}\n\nTRANSCRIPT\n${transcript || '[No audio supplied.]'}\n\nMATERIALS\n${context}` }, ...files] }] }) });
