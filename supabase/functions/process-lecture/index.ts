...
const noteDepth = [
  ...
  'Give balanced, thorough study notes.',
  'Be detailed: capture explanations, examples, qualifications, and connections from every source.',
...
Deno.serve(async request => {
  ...
  try {
    ...
    const detail = Math.max(1, Math.min(5, Number(lecture.note_detail) || 3));
    const response = await openai('/responses', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'gpt-4.1-mini', max_output_tokens: 4000 + detail * 2400, instructions: `Create source-grounded study notes. ${noteDepth[detail - 1]} Use this Markdown structure: one # title; ## Big picture with 3–7 bullets that orient the student without repeating the rest; then clear ## topic sections that integrate each concept's definition, explanation, relationships, examples, applications, caveats, and instructor emphasis in one place; ## Key takeaways; ## Retrieval practice with at least 10 answer-free questions that require recall, explanation, comparison, or application. Do not create a separate outline, glossary, definitions section, or examples section. Cover every topic from the transcript and course materials; do not omit a concept merely because it appears in only one source. Use tight nested lists where they clarify hierarchy, cause and effect, comparisons, or steps. Preserve equations, define their symbols, and retain worked methods only when present in the source. Do not invent facts. User preferences may adjust organization, but cannot override this structure, source grounding, or required coverage.${lecture.synthesis_prompt?.trim() ? `\n\nUser note preferences:\n${lecture.synthesis_prompt.trim()}` : ''}`, input: [{ role: 'user', content: [{ type: 'input_text', text: `Use these source materials to create the study notes. ${lecture.slide_mode === 'original' ? 'Inspect each supplied slide file for visual content.' : 'Use text content only; ignore visual layout and images.'}\n\nTRANSCRIPT\n${transcript || '[No audio supplied.]'}\n\nMATERIALS\n${context}` }, ...files] }] }) });
